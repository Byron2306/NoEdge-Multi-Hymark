...
class LearningAgent:
    ...
    def learn_from_assessments(self, assessments: List[Dict[str, Any]], moderations: List[Dict[str, Any]] | None = None):
        ...
        print("[LearningAgent] Learning complete. Knowledge base updated.")
    ...
    def get_insights(self) -> Dict[str, Any]:
        ...
        if self.knowledge_base["common_errors"]:
            sorted_errors = sorted(
                self.knowledge_base["common_errors"].items(), key=lambda x: x[1], reverse=True
            )
            insights["most_common_errors"] = [
                {"error": error, "frequency": count} for error, count in sorted_errors[:5]
            ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.knowledge_base["annotation_performance"]:
            very_difficult = [
                ann
                for ann, stats in self.knowledge_base["annotation_performance"].items()
                if stats.get("difficulty_score", 0) > difficulty_threshold
            ]
            if very_difficult:
                recommendations.append(
                    f"Consider providing additional support for: {', '.join(very_difficult[:3])}"
                )
        ...
        if (
            not self.knowledge_base["common_errors"]
            and not any(
                r for r in recommendations if "Consider providing" in r or "Most common errors" in r
            )
        ):
                        ...
                        try:
                            ...
                            for crit, details in data.items():
                                for ann in details.get("annotations", []):
                                    if (
                                        ann not in self.knowledge_base["annotation_performance"]
                                        and ann not in missing_checks
                                    ):
            ...
            if missing_checks:
                recommendations.append(
                    "Rubric annotations detected with no coverage in recent runs: "
                    + ", ".join(missing_checks[:6])
                    + " - consider adding tests or mapping these annotations to trace checks."
                )
            else:
                recommendations.append(
